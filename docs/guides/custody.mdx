...
3. **Track Balances**  
   Use the [Balances endpoint](/api/fetch-the-balance-of-an-internal-custodial-address-for-a-specific-token) to query stablecoin holdings across all wallets and chains.
4. **Initiate Payouts**  
...
[`POST /accounts`](/api/create-a-customer-account) - Register an account

[`GET /addresses`](/api/fetch-all-addresses) - Fetch custodial wallet addresses. Custodial wallets have type `internal`

[`GET /balances`](/api/fetch-the-balance-of-an-internal-custodial-address-for-a-specific-token) - View wallet balances

[`POST /transfers`](/api/create-a-new-transfer) - Initiate transfers and payouts
